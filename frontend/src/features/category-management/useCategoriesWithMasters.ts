...
export const useCategoriesWithMasters = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const categoriesWithMasters: CategoryWithMaster[] = categoriesArray.map(category => {
        const master = mastersMap.get(category.categoryMasterId || category.category_master_id);
        if (!master) {
