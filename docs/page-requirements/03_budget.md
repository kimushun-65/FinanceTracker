...
3. **予算実績取得API**
   - エンドポイント: `/api/budgets/performance`
   - パラメータ: year_month
   - レスポンス: カテゴリ別予算・実績対比

4. **AI予算提案API**
   - エンドポイント: `/api/budgets/ai-suggestion`
   - パラメータ: 
     - target_month: 対象月
     - optimization_type: 節約重視/バランス重視
   - レスポンス: AI生成の予算案と根拠

### データベース参照
- `budgets`テーブル: 予算データ
- `budget_suggestions`テーブル: AI提案履歴
- `transactions`テーブル: 実績計算用
